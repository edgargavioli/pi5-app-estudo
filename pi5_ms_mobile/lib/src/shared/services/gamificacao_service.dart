...
class GamificacaoService {
  ...
  static Future<int> calcularXpTotal() async {
    try {
      ...
      } // XP por sessões de estudo realizadas
      for (final sessao in sessoes) {
        if (sessao.finalizada && sessao.questoesAcertadas != 0) {
          // XP por questões acertadas
          xpTotal += sessao.questoesAcertadas * XP_POR_QUESTAO_ACERTADA;
          ...
          if (sessao.totalQuestoes > 0) {
            final percentual =
                (sessao.questoesAcertadas / sessao.totalQuestoes) * 100;
            final bonus = _calcularBonusDesempenho(percentual.round());
            xpTotal += bonus;
          }
        }
      }
      ...
      // Em caso de erro, retornar 0 em vez de dados em cache para evitar dados incorretos
      return 0;
    }
  }
  ...
  static Future<Map<String, dynamic>> obterEstatisticasCompletas() async {
    try {
      ...
      final sessoesFinalizadas = sessoes.where((s) => s.finalizada).length;
      ...
      // Questões acertadas das sessões
      int totalQuestoes = 0;
      int totalAcertos = 0;
      for (final sessao in sessoes) {
        if (sessao.finalizada) {
          totalQuestoes += sessao.totalQuestoes;
          totalAcertos += sessao.questoesAcertadas;
        }
      }

      // Desempenho médio das sessões
      double desempenhoMedio = 0.0;
      final sessoesComResultado =
          sessoes.where((s) => s.finalizada && s.totalQuestoes > 0).toList();
      if (sessoesComResultado.isNotEmpty) {
        final somaPercentuais = sessoesComResultado.fold<double>(
          0.0,
          (sum, sessao) =>
              sum + ((sessao.questoesAcertadas / sessao.totalQuestoes) * 100),
        );
        desempenhoMedio = somaPercentuais / sessoesComResultado.length;
      }
  ...
  /// Limpa dados salvos localmente (útil ao fazer logout/login com novo usuário)
  static Future<void> limparDadosLocais() async {
    try {
      ...
      print('Dados locais de gamificação limpos');
    } catch (e) {
  ...
  static int _xpNecessarioParaNivel(int nivel) {
    ...
    for (int i = 2; i <= nivel; i++) {
      final xpNivel =
          (XP_BASE_POR_NIVEL * pow(MULTIPLICADOR_NIVEL, i - 2)).round();
      xpTotal += xpNivel;
    }
