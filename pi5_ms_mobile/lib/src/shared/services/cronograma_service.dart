...
class CronogramaService {
  ...
  static Future<Map<DateTime, List<dynamic>>> obterEventosDoMes(DateTime data) async {
    try {
      ...
      for (final evento in eventosLista) {
        final dataEvento = DateTime(evento.data.year, evento.data.month, evento.data.day);
  ...
  static Future<List<Prova>> obterProximasProvas({int dias = 30}) async {
    try {
      ...
      return todasProvas
          .where((prova) => 
              prova.data.isAfter(agora) && 
              prova.data.isBefore(limite))
          .toList()
  ...
  static Future<List<Prova>> _obterProvasDoDia(DateTime data) async {
    final todasProvas = await ProvaService.listarProvas();
    return todasProvas.where((prova) => 
  ...
  static Future<List<SessaoEstudo>> _obterSessoesDoDia(DateTime data) async {
    ...
    return todasSessoes.where((sessao) => 
  ...
  static Future<List<Evento>> _obterEventosDoDia(DateTime data) async {
    ...
    return todosEventos.where((evento) => 
  ...
  static Future<Map<String, int>> obterEstatisticas() async {
    try {
      ...
      final proximosEventos = eventos.where((e) => e.data.isAfter(agora)).length;
      final sessoesHoje = sessoes.where((s) => 
          s.tempoInicio != null &&
